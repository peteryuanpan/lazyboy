...
const char adress[] = {"/Users/peteryuanpan/Chess/17-data.txt"};
const int N = 10010;
int JiangX[] = {1, -1, 0, 0};
int JiangY[] = {0, 0, 1, -1};
...
private:
    int id, red, fx, fy, Jx, Jy, jx, jy, zx, zy, x, y, colorType;
    string str;
public://基本判断集
...
private://分别判断车、马、炮、兵能否将到将
    ...
    bool Ma_Jiang(int t){
        for(int k = 0; k < 8; k++){
            zx = jx + MaX[k], zy = jy + MaY[k];
            if(d[id].At(zx, zy) == str[t] && Ma_object(id, zx, zy, jx, jy)) return true;
        }
        return false;
    }
    bool Pao_Jiang(int t){
        for(int i = 0; i < nrow; i++){
...
public://判断是否是一种将军局面
    bool Fuck_Jiang(int nowid, bool red1){
        id = nowid, red = red1;//red为true表示红将黑，否则轮黑将红
        
        Jiang_xy(id, Jx, Jy, 'J'), Jiang_xy(id, jx, jy, 'j');
        if(Jvsj(id, Jx, Jy, jx, jy)) return true;//将对将
        
        str = red? "CMPB" : "cmpb";//攻击棋子
        if(!red) swap(Jx, jx), swap(Jy, jy);
        
        if(Che_Jiang(0)) return true;
        else if(Ma_Jiang(1)) return true;
        else if(Pao_Jiang(2)) return true;
        else if(Bin_Jiang(3)) return true;
        else return false;
    }
...
public://移动(fx, fy)到(jx, jy)之间的第k个棋子(且之间有n个棋子)，以达到将军或者躲将的目的
    void MoveToFuckOrAvoid(int nowid, bool red1, int fx1, int fy1, int jx1, int jy1, int n, int k){
        ...
        if(!red) fx = fy = -1;
        char c = d[id].At(x, y);
        if(c == str[0]) Move_jiang();
        else if(c == str[1]) Move_che();
        else if(c == str[2]) Move_ma();
        else if(c == str[3]) Move_pao();
        else if(c == str[4]) Move_xiang();
        else if(c == str[5]) Move_shi();
        else if(c == str[6]) Move_bin();
    }
}bjo;
...
public://处理红棋将军
    void Fuck_Black(int nowid, int x1, int y1){
        id = nowid, x = x1, y = y1;
...
class BlackAvoidRed{
private:
    int id, jx, jy, fx, fy, zx, zy;
    string str;
private://判断危险红棋子能否被吃掉
    void Eat_fucker(int fx, int fy){
        str = "jcmpxsb";
        for(int i = 0; i < nrow; i++){
            for(int j = 0; j < ncol; j++){
                ...
                else if(c == str[6] && bjo.Bin_object(id, i, j, fx, fy)) t = 6;
                else continue;
